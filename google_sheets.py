...
class GoogleSheetsClient:
    ...
    def update_result(self, spreadsheet_url: str, row_number: int,
        ...
        output_columns = ['file_location', 'record_count', 'status', 'timestamp']
        ...
        worksheet.update_cell(row_number, headers.index('file_location') + 1, file_location)
        worksheet.update_cell(row_number, headers.index('record_count') + 1, record_count)
        worksheet.update_cell(row_number, headers.index('status') + 1, status)
        worksheet.update_cell(row_number, headers.index('timestamp') + 1, timestamp)
